...
std::vector<Type *> RISCV64FunctionPrototypeDiscoverer::discoverArgumentTypes() const {
  ...
  for (MachineBasicBlock &MBB : MF) {
    // Check if the instruction moves the argument to a local register or if it stores the
    // argument on the stack. Only check the first move/store instructions after the prolog.
